...
IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t CS, int8_t RST) : Adafruit_PCD8544(SCLK, DIN, DC, CS, RST), Clavier_5T_FreeRTOS( _fausseUpdate){
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t RST) : Adafruit_PCD8544(SCLK, DIN, DC, RST), Clavier_5T_FreeRTOS(_fausseUpdate) {
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t DC, int8_t CS,
  int8_t RST): Adafruit_PCD8544(DC, CS, RST), Clavier_5T_FreeRTOS(_fausseUpdate){
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t CS, int8_t RST,void(*_fct_update)(void)) : Adafruit_PCD8544(SCLK, DIN, DC, CS, RST), Clavier_5T_FreeRTOS(_fct_update) {
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t RST,void(*_fct_update)(void)) : Adafruit_PCD8544(SCLK, DIN, DC, RST), Clavier_5T_FreeRTOS(_fct_update) {
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_FreeRTOS::IO_PCD8544_5T_FreeRTOS(int8_t DC, int8_t CS, int8_t RST,
  void(*_fct_update)(void)): Adafruit_PCD8544(DC, CS, RST), Clavier_5T_FreeRTOS(_fct_update) {
    //widgets=Widgets(this);
    return;
}

void IO_PCD8544_5T_FreeRTOS::display(void){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::display();
  taskEXIT_CRITICAL();
}

void IO_PCD8544_5T_FreeRTOS::command(uint8_t c){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::command(c);
  taskEXIT_CRITICAL();
}
void IO_PCD8544_5T_FreeRTOS::data(uint8_t c){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::data(c);
  taskEXIT_CRITICAL();
}

void IO_PCD8544_5T_FreeRTOS::setUpdateFct(FonctionUpdate _fctUpdate){
  fctUpdate=_fctUpdate;
}

void IO_PCD8544_5T_FreeRTOS::onOffEcran(bool etat){
...
bool IO_PCD8544_5T_FreeRTOS::etatEcran(void){
  #ifdef PORT_LED_ECRAN
...
void IO_PCD8544_5T_FreeRTOS::begin(uint8_t contrast, uint8_t bias){
...
IO_PCD8544_5T_SPI_software_FreeRTOS::IO_PCD8544_5T_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t CS, int8_t RST) : PCD8544_SPI_Software(SCLK, DIN, DC, CS, RST), Clavier_5T_FreeRTOS(_fausseUpdate){
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_SPI_software_FreeRTOS::IO_PCD8544_5T_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t RST) : PCD8544_SPI_Software(SCLK, DIN, DC, RST), Clavier_5T_FreeRTOS(_fausseUpdate) {
    //widgets=Widgets(this);
    return;
}
...
IO_PCD8544_5T_SPI_software_FreeRTOS::IO_PCD8544_5T_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t CS, int8_t RST,void(*_fct_update)(void)) : PCD8544_SPI_Software(SCLK, DIN, DC, CS, RST), Clavier_5T_FreeRTOS(_fct_update) {
    //widgets=Widgets(this);
    return;
}

IO_PCD8544_5T_SPI_software_FreeRTOS::IO_PCD8544_5T_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t RST,void(*_fct_update)(void)) : PCD8544_SPI_Software(SCLK, DIN, DC, RST), Clavier_5T_FreeRTOS(_fct_update) {
    //widgets=Widgets(this);
    return;
}

void IO_PCD8544_5T_SPI_software_FreeRTOS::display(void){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::display();
  taskEXIT_CRITICAL();
}

void IO_PCD8544_5T_SPI_software_FreeRTOS::command(uint8_t c){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::command(c);
  taskEXIT_CRITICAL();
}
void IO_PCD8544_5T_SPI_software_FreeRTOS::data(uint8_t c){
  taskENTER_CRITICAL();
    Adafruit_PCD8544::data(c);
  taskEXIT_CRITICAL();
}

void IO_PCD8544_5T_SPI_software_FreeRTOS::setUpdateFct(FonctionUpdate _fctUpdate){
  fctUpdate=_fctUpdate;
}

void IO_PCD8544_5T_SPI_software_FreeRTOS::onOffEcran(bool etat){
  #ifdef PORT_LED_ECRAN
  if(etat) PORT_LED_ECRAN |= (1 << BIT_LED_ECRAN); // on ralume l'écran
  else PORT_LED_ECRAN &= ~(1 << BIT_LED_ECRAN); // éteint l'écran
  #else
...
bool IO_PCD8544_5T_SPI_software_FreeRTOS::etatEcran(void){
  #ifdef PORT_LED_ECRAN
    return ((PORT_LED_ECRAN&(1<<BIT_LED_ECRAN)) != 0);
  #else
    return _etatEcran;
...
void IO_PCD8544_5T_SPI_software_FreeRTOS::begin(uint8_t contrast, uint8_t bias){
...
retourSelect IO_PCD8544_5T_FreeRTOS::menuListe(char* chaine, uint8_t nb, uint8_t taille, uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t actual, uint8_t permissions) {
  //      affiche une liste est retourne le numéro de l'item selectionné
  //      liste est un tableau de pointeur vers des chaine comprenant la chaine de caractère de l'item
  //      nb est le nombre d'élément dans la liste principale
  //      taille donne la taille des élements de la chaine /!\ y compris le bit de fin de chaine /!\.
...
retourSelect IO_PCD8544_5T_FreeRTOS::menuListeV2(void (*callback)(uint8_t, char*, uint8_t, void*), void* arg, uint8_t nb, uint8_t taille, uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t actual, uint8_t permissions) {
  //      affiche une liste est retourne le numéro de l'item selectionné
  //      liste est un tableau de pointeur vers des chaine comprenant la chaine de caractère de l'item
  //      nb est le nombre d'élément dans la liste principale
  //      taille donne la taille des élements de la chaine /!\ y compris le bit de fin de chaine /!\.
...
retourSelect IO_PCD8544_5T_FreeRTOS::menuListeOuiNon(uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t permissions) {
...
retourSelect IO_PCD8544_5T_SPI_software_FreeRTOS::menuListeOld(char* chaine, uint8_t nb, uint8_t taille, uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t actual, uint8_t permissions) {
  //      affiche une liste est retourne le numéro de l'item selectionné
  //      liste est un tableau de pointeur vers des chaine comprenant la chaine de caractère de l'item
  //      nb est le nombre d'élément dans la liste principale
  //      taille donne la taille des élements de la chaine /!\ y compris le bit de fin de chaine /!\.
...
retourSelect IO_PCD8544_5T_SPI_software_FreeRTOS::menuListe(char* chaine, uint8_t nb, uint8_t taille, uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t actual, uint8_t permissions) {
//      affiche une liste est retourne le numéro de l'item selectionné
//      liste est un tableau de pointeur vers des chaine comprenant la chaine de caractère de l'item
//      nb est le nombre d'élément dans la liste principale
//      taille donne la taille des élements de la chaine /!\ y compris le bit de fin de chaine /!\.
...
retourSelect IO_PCD8544_5T_SPI_software_FreeRTOS::menuListeV2(void (*callback)(uint8_t, char*, uint8_t, void*), void* arg, uint8_t nb, uint8_t taille, uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t actual, uint8_t permissions) {
//      affiche une liste est retourne le numéro de l'item selectionné
//      liste est un tableau de pointeur vers des chaine comprenant la chaine de caractère de l'item
//      nb est le nombre d'élément dans la liste principale
//      taille donne la taille des élements de la chaine /!\ y compris le bit de fin de chaine /!\.
...
retourSelect IO_PCD8544_5T_SPI_software_FreeRTOS::menuListeOuiNon(uint8_t yMin, uint8_t yMax, uint8_t xMin, uint8_t xMax, uint8_t hLigne, uint8_t depart, uint8_t textSize, uint8_t permissions) {
  return menuListeV2(_creeLigneMenuListeOuiNon, (void*)&yMin, 2, 4, yMin, yMax, xMin, xMax, hLigne, depart, textSize, 0, permissions);
}
*/
