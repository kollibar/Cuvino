#ifndef __IO_PCD8544_3TOUCHES_FREERTOS_H__
#define __IO_PCD8544_3TOUCHES_FREERTOS_H__
...
class IO_PCD8544_3touches_FreeRTOS : virtual public Adafruit_PCD8544, public Clavier_3T_Analog_FreeRTOS{
public:
  IO_PCD8544_3touches_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t CS, int8_t RST,FonctionUpdate _fctUpdate);
  // Software SPI with CS tied to ground.  Saves a pin but other pins can't be shared with other hardware.
  IO_PCD8544_3touches_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t RST,FonctionUpdate _fctUpdate);
  ...
  IO_PCD8544_3touches_FreeRTOS(int8_t DC, int8_t CS, int8_t RST,FonctionUpdate _fctUpdate);

  // ***** Sans la fonction d'update du clavier /!\ dans ce cas, les attentes clavier seront bloquante /!\.
  IO_PCD8544_3touches_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t CS, int8_t RST);
  // Software SPI with CS tied to ground.  Saves a pin but other pins can't be shared with other hardware.
  IO_PCD8544_3touches_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t RST);
  // Hardware SPI based on hardware controlled SCK (SCLK) and MOSI (DIN) pins. CS is still controlled by any IO pin.
  // NOTE: MISO and SS will be set as an input and output respectively, so be careful sharing those pins!
  IO_PCD8544_3touches_FreeRTOS(int8_t DC, int8_t CS, int8_t RST);
...
class IO_PCD8544_3touches_SPI_software_FreeRTOS: public PCD8544_SPI_Software, public Clavier_3T_Analog_FreeRTOS{
public:
  IO_PCD8544_3touches_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t CS, int8_t RST,FonctionUpdate _fctUpdate);
  // Software SPI with CS tied to ground.  Saves a pin but other pins can't be shared with other hardware.
  IO_PCD8544_3touches_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t RST,FonctionUpdate _fctUpdate);

  // ***** Sans la fonction d'update du clavier /!\ dans ce cas, les attentes clavier seront bloquante /!\.
  IO_PCD8544_3touches_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t CS, int8_t RST);
  // Software SPI with CS tied to ground.  Saves a pin but other pins can't be shared with other hardware.
  IO_PCD8544_3touches_SPI_software_FreeRTOS(int8_t SCLK, int8_t DIN, int8_t DC, int8_t RST);
