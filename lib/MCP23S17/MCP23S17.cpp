...
MCP23S17::MCP23S17(void (*fct)(uint8_t), uint8_t address, SPIClass* spi){
  _address = (address << 1);
  _select  = 255;
  _error   = MCP23S17_OK;
  _mySPI   = spi;
...
bool      MCP23S17::writeRegWord(uint8_t reg, uint16_t value){
  _error = MCP23S17_OK;
  ...
  if (_hwSPI)
  {
    ...
    _mySPI->transfer(reg);
	  ...
	  #else
		_mySPI->transfer((uint8_t)(value >> 8));
		_mySPI->transfer((uint8_t)(value & 0xFF));
	  #endif
  }
  else
  {
    ...
    swSPI_transfer(reg);
		swSPI_transfer((uint8_t)(value >> 8));
		swSPI_transfer((uint8_t)(value & 0xFF));
  }
  if( _select != 255 )
    ::digitalWrite(_select, HIGH);
  else
...
uint16_t  MCP23S17::readRegWord(uint8_t reg){
  uint16_t rv = 0;
