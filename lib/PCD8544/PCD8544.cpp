...
Adafruit_PCD8544::Adafruit_PCD8544(int8_t SCLK, int8_t DIN, int8_t DC,
                                   int8_t CS, int8_t RST) : Adafruit_GFX_NB8bits(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
Adafruit_PCD8544::Adafruit_PCD8544(int8_t SCLK, int8_t DIN, int8_t DC,
                                   int8_t RST) : Adafruit_GFX_NB8bits(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
Adafruit_PCD8544::Adafruit_PCD8544(int8_t DC, int8_t CS, int8_t RST):
  Adafruit_GFX_NB8bits(LCDWIDTH, LCDHEIGHT) {
...
void Adafruit_PCD8544::drawPixel(int8_t x, int8_t y, uint8_t color) {
  ...
  // x is which column
  if (color)
    pcd8544_buffer[(int16_t)x + ((int16_t)y / 8)*LCDWIDTH] |= _BV(y % 8);
  else
    pcd8544_buffer[(int16_t)x + ((int16_t)y / 8)*LCDWIDTH] &= ~_BV(y % 8);

  updateBoundingBox(x, y, x, y);
}
void Adafruit_PCD8544::invertPixel(int8_t x, int8_t y) {
  if ((x < 0) || (x >= _width) || (y < 0) || (y >= _height))
    return;
...
void Adafruit_PCD8544::invertRect(int8_t x, int8_t y, int8_t w, int8_t h) {
  if ((x + w < 0) || (x > _width) || (y + h < 0) || (y > _height))
    return;
  ...
  if ((x < 0) || (x + w > LCDWIDTH) || (y < 0) || (y + h > LCDHEIGHT))
    return;
  if (x < 0)x = 0;
  if (y < 0)y = 0;
  if (x + w > _width) w = _width - x - 1;
  if (y + h > _height) y = _height - y - 1;

  // x is which column
  for(int16_t i=x;i<x+w;++i){ // pour chaque colonne
    int16_t j=y;
    while(j<y+h && j%8!=0){ // debut (octets non complets)
      pcd8544_buffer[i + (j / 8)*LCDWIDTH] ^= _BV(j % 8);
      ++j;
    }
    int16_t octet=i + (j / 8)*LCDWIDTH;
    while((j+8)<y+h){ // milieu, (octets complets)
      pcd8544_buffer[octet] ^= B11111111;
      octet+=LCDWIDTH;
      j+=8;
    }
  ...
  updateBoundingBox(x, y, x+w-1, y+h-1);
}
...
uint8_t Adafruit_PCD8544::getPixel(int8_t x, int8_t y) {
  ...
  return (pcd8544_buffer[(int16_t)x + ((int16_t)y / 8) * LCDWIDTH] >> (y % 8)) & 0x1;
}
...
bool Adafruit_PCD8544::isHardwareSPI() const{
...
PCD8544_SPI_Software::PCD8544_SPI_Software(int8_t SCLK, int8_t DIN, int8_t DC,
  int8_t RST) : Adafruit_PCD8544(SCLK, DIN, DC, RST){
}

void PCD8544_SPI_Software::begin(uint8_t contrast, uint8_t bias) {
...
bool PCD8544_SPI_Software::isHardwareSPI() const{ return false;}
