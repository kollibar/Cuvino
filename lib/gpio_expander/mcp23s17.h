...
public:
	mcp23s17(csFct fct,const uint8_t haenAdrs);//any pin,0x20....0x27
	mcp23s17(const uint8_t csPin,const uint8_t haenAdrs);//any pin,0x20....0x27
	mcp23s17(csFct fct,const uint8_t haenAdrs,uint32_t spispeed);//for SPI transactions
	...
	void 			postSetup(const int8_t csPin, csFct fct,const uint8_t haenAdrs,uint32_t spispeed=0);//used with other libraries only
